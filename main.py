...
def setup():
    try:
        ...
        if os.path.isfile("record.exe") == False:
            ...
            l = [{'Target': 'Not Set', 'DateOfFirstRun': time.strftime(('%d'+'/'+'%m'+'/'+'%Y'), time.localtime(
            ))}, ('Time_Set(Minutes)', 'Completed', 'Date', 'Apps_Blocked', 'Points_Earned', 'Total_Points')]
            pickle.dump(l, f)
            f.close()
...
def check_requirements():
    if os.path.isfile("congrats.mp3") == True and os.path.isfile("goingtostart.mp3") == True and os.path.isfile("start.mp3") == True and os.path.isfile("record.exe") == True:
        return
...
def start_blocker(futuretime=0):
    try:
        os.system('cls')
        os.system('color 0a')
...
# saving the scores --- 1
        ...
        l1 = ((t_for_score/60), 'NO', time.strftime(('%d'+'/'+'%m'+'/'+'%Y'),
              time.localtime()), apps, (score+futuretime)*(-1), total_score)
        l.append(l1)
        pickle.dump(l, f)
        f.close()
...
# future time
        ...
        if futuretime > 0:
            os.system('cls')
            os.system('color 0f')
            print('The blocker will start ', (futuretime/60), 'minutes after',
                  time.strftime(('%I'+':'+'%M'+' '+'%p'), time.localtime()))
            if futuretime >= 900:
                time.sleep(futuretime-900)
                os.system("goingtostart.mp3")
                os.system("cls")
                print('The blocker will start 15 minutes after', time.strftime(
...
# running the main code
        ...
        while time.time() < t_end:
            ...
            print(
                '.\/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/.')
            print("Time Set :", round((t_for_score/60), 3), "minutes or",
                  round((t_for_score/3600), 3), "hours from", starttime)
            print("Score that can be earned on Completion :", (score+futuretime))
            print(
                './\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\.')
            print(
...
# saving the scores --- 2
        ...
        if type(l[-1][-1]) == str:
            ...
            total_score = int(l[-1][-1])+(score+futuretime)
        l1 = ((t_for_score/60), 'YES', time.strftime(('%d'+'/'+'%m'+'/'+'%Y'),
              time.localtime()), apps, (score+futuretime), total_score)
        l.append(l1)
        pickle.dump(l, f)
        f.close()
...
# 2
        ...
        try:
            ...
            print(
                "You will get extra points if you don't cancel the the blocker in between.")
            print(
                '========================================================================================')
...
# 3
        ...
        print('========================================================================================')
        print('.\/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/ \/.')
        print('Total Points : ', l[-1][-1] if type(l[-1]
              [-1]) == int else print('No Scores Yet'))
        print('./\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\ /\.')
        print('========================================================================================')
        print('========================================================================================')
        enter = input('Press Enter To Continue.')
...
# 4
        ...
        if type(l[-1][-1]) == int:
            ...
            print("You have focused :", round(hrs, 2),
                  "hours", "or", round(days, 2), "days")
        ...
        try:
            ...
            if type(l[0]['Target']) == int and type(l[-1][-1]) == int:
                ...
                if l[0]['Target'] > int(l[-1][-1]):
                    ...
                    print('Score Remaining to Target :', remaining,
                          ' (', round(remaining/3600, 1), 'hrs )')
